...
@section('content')
    ...
    <div class="card bradius p-2">
        ...
        <form method="POST" action="{{ route('education.store') }}" enctype="multipart/form-data" id="educationForm">
            ...
            <textarea class="bradius" name="info" cols="30" rows="10"
                      style="width: 98%; margin: 15px">{{ $info->info }}</textarea>
            <div class="ml-3 mb-3">
                <h6>Images:</h6>
                    ...
                    @foreach ($info_images as $image)
                        <div class="p-2 mx-2">
                            ...
                            <i class="fa fa-times-circle position-absolute top-0 end-0"
                               ...
                               onclick="removeImage(this,{{ $image->id }},{{ $info->id }})"></i>
                        </div>
                    @endforeach
                </div>
                <div class="p-2">
    ...
    <script>
        ...
        form.addEventListener('submit', function (e) {
            ...
            formData.append('info', document.querySelector('[name="info"]').value);
